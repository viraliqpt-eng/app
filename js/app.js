// Banco de Dados Oficial de 100 Temas Absurdos do SHITAGRAM™
        ...
        function switchTab(tabId, options = {}) {
            const protectedTabs = ['feed', 'ai-creator', 'profile'];
            if (!options.skipAuth && protectedTabs.includes(tabId) && !getCurrentUser()) {
                pendingTabAfterAuth = tabId;
                openAuth('register');
                showAuthMessage('Cria conta ou entra primeiro para aceder a esta área.', 'info');
                return;
            }
        ...
        function enterAppFlow(tabId) {
            if (!getCurrentUser()) {
                pendingTabAfterAuth = tabId || 'profile';
                openAuth('register');
                return;
            }
            switchTab(tabId || 'feed');
        }
        ...
        // Login e registo local para protótipo
        let pendingTabAfterAuth = 'profile';

        function getUsersStore() {
            return JSON.parse(localStorage.getItem('shitagram_users') || '{}');
        }

        function saveUsersStore(users) {
            localStorage.setItem('shitagram_users', JSON.stringify(users));
        }

        function getCurrentUser() {
            return JSON.parse(localStorage.getItem('shitagram_current_user') || 'null');
        }

        function setCurrentUser(user) {
            localStorage.setItem('shitagram_current_user', JSON.stringify(user));
            localStorage.setItem('shitagram_profile', JSON.stringify({ name: user.name, avatar: user.avatar || '' }));
            loadProfileRealtime();
            updateAuthUI();
        }
        ...
        function setAuthMode(mode) {
            ...
            loginTab.className = isRegister ? 'flex-1 py-3 rounded-xl text-sm font-black transition-all text-orange-300' : 'flex-1 py-3 rounded-xl text-sm font-black transition-all bg-orange-600 text-white';
            registerTab.className = isRegister ? 'flex-1 py-3 rounded-xl text-sm font-black transition-all bg-orange-600 text-white' : 'flex-1 py-3 rounded-xl text-sm font-black transition-all text-orange-300';
        ...
        function showAuthMessage(message, type = 'info') {
            ...
            if (type === 'error') {
                el.className = 'mt-5 text-xs font-bold rounded-xl px-4 py-3 border bg-red-500/10 border-red-500/30 text-red-200';
            } else if (type === 'success') {
                el.className = 'mt-5 text-xs font-bold rounded-xl px-4 py-3 border bg-emerald-500/10 border-emerald-500/30 text-emerald-200';
            } else {
                el.className = 'mt-5 text-xs font-bold rounded-xl px-4 py-3 border bg-orange-500/10 border-orange-500/30 text-orange-200';
            }
        }
        ...
        function handleRegister(event) {
            ...
            if (users[email]) {
                showAuthMessage('Já existe uma conta com este email. Entra com a palavra-passe.', 'error');
                setAuthMode('login');
                document.getElementById('login-email').value = email;
                return;
            }
            const cleanName = name || `Reclamante_${Math.random().toString(36).slice(2, 8)}`;
            const user = {
                id: `user_shitagram_${Math.random().toString(36).slice(2, 9)}`,
                name: cleanName,
                email,
                password,
                avatar,
            ...
            setCurrentUser(user);
            showAuthMessage('Conta criada com sucesso. A abrir o teu perfil...', 'success');
            showCustomNotification('Conta criada', 'Bem-vindo ao SHITAGRAM™.', 'success');
            setTimeout(() => switchTab('profile', { skipAuth: true }), 450);
        }
        ...
        function handleLogin(event) {
            ...
            if (!user || user.password !== password) {
                showAuthMessage('Email ou palavra-passe incorretos.', 'error');
                return;
            }
            setCurrentUser(user);
            showAuthMessage('Sessão iniciada. A entrar...', 'success');
            showCustomNotification('Sessão iniciada', `Olá, ${user.name}!`, 'success');
            setTimeout(() => switchTab(pendingTabAfterAuth || 'profile', { skipAuth: true }), 350);
        }

        function logoutUser() {
            localStorage.removeItem('shitagram_current_user');
            updateAuthUI();
            showCustomNotification('Sessão terminada', 'Voltaste à página inicial.', 'info');
            switchTab('home', { skipAuth: true });
        }
        ...
        function updateAuthUI() {
            ...
            if (registerBtn) registerBtn.classList.toggle('hidden', !!user);
            if (logoutBtn) logoutBtn.classList.toggle('hidden', !user);
            if (logoutBtn && user) logoutBtn.classList.add('flex');
            const authEl = document.getElementById('auth-status');
            if (authEl && user) authEl.textContent = user.name;
            if (authEl && !user) authEl.textContent = 'Visitante';
        }
        ...
        function loadProfileRealtime() {
            ...
            const saved = current ? { name: current.name, avatar: current.avatar || '' } : JSON.parse(localStorage.getItem('shitagram_profile') || '{}');
            applyProfileRealtime(saved);
            if (current) {
                const idEl = document.getElementById('my-profile-id');
                if (idEl) idEl.textContent = `ID: ${current.id}`;
            }
        }
        ...
        function saveProfileRealtime() {
            ...
            if (current) {
                ...
                localStorage.setItem('shitagram_current_user', JSON.stringify(current));
                const users = getUsersStore();
                if (users[current.email]) {
                    users[current.email].name = profile.name;
                    users[current.email].avatar = profile.avatar;
                    saveUsersStore(users);
                }
            }
