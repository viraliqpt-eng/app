...
import { walletMiddleware } from './src/middleware/wallet.js';
...
const creditStore = new CreditStore(config.creditsFile, config.freeCredits);
...
app.use(walletMiddleware({ secure: config.nodeEnv === 'production' }));
...
app.use('/api/credits', createCreditsRouter({
  store: creditStore,
  generationCost: config.generationCreditCost
}));
app.use('/api/generations', createGenerationsRouter({
