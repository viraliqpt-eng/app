...
.edge-tools{
  ...
  border-radius:22px;
  padding:24px;
  background:linear-gradient(180deg,rgba(255,122,0,.08),rgba(0,0,0,.22));
}
.edge-tools h2{
  margin:0 0 12px;
  font-size:28px;
...
@media(max-width:720px){
  .edge-grid,.edge-status{grid-template-columns:1fr}
}
</style>
</head>
<body>
...
(function(){
  ...
  window.addEventListener("beforeinstallprompt", (event) => {
    ...
    deferredInstallPrompt = event;
    const pwa = document.getElementById("pwaStatusText");
    if (pwa) pwa.textContent = "Pronta para instalar";
  });

  window.addEventListener("appinstalled", () => {
    const pwa = document.getElementById("pwaStatusText");
    if (pwa) pwa.textContent = "Instalada";
  });

  window.installShitagram = async function(){
    if (deferredInstallPrompt) {
      deferredInstallPrompt.prompt();
      const choice = await deferredInstallPrompt.userChoice;
      const pwa = document.getElementById("pwaStatusText");
      if (pwa) pwa.textContent = choice.outcome === "accepted" ? "Instalação aceite" : "Instalação recusada";
      deferredInstallPrompt = null;
      return;
    }

    window.copyInstallInstructions && window.copyInstallInstructions();
    alert("Se o botão de instalação não aparecer, usa o menu do Chrome/Edge: ⋮ → Guardar e partilhar → Instalar página como app.");
  };

  window.copyInstallInstructions = function(){
    const text = `Como instalar o SHITAGRAM™ no PC:

Chrome/Edge:
1. Abre o site.
2. Clica no menu ⋮.
3. Vai a Guardar e partilhar.
4. Escolhe Instalar página como app.
...
Partilhar → Adicionar ao ecrã principal.`;
  ...
  window.addEventListener("load", async () => {
    ...
    const pwaStatus = document.getElementById("pwaStatusText");

    if (manifestStatus) {
      manifestStatus.textContent = document.querySelector('link[rel="manifest"]') ? "Encontrado" : "Não encontrado";
    }

    if (!("serviceWorker" in navigator)) {
      if (swStatus) swStatus.textContent = "Não suportado";
      return;
    }
    ...
    try {
      ...
      if (pwaStatus && !pwaStatus.textContent.trim()) pwaStatus.textContent = "Pronta para instalar";
      console.log("[SHITAGRAM] Service Worker ativo:", reg.scope);
    } catch (err) {
      console.error("[SHITAGRAM] Erro no Service Worker:", err);
      if (swStatus) swStatus.textContent = "Erro ao ativar";
      if (pwaStatus) pwaStatus.textContent = "Verifica service-worker.js";
    }
  });
})();
</script>
...
(function(){
  ...
  window.addEventListener("beforeinstallprompt", (event) => {
    event.preventDefault();
    deferredInstallPrompt = event;
    setText("edgeInstallStatus", "Pronto para instalar");
    setText("pwaStatusText", "Pronta para instalar");
  });

  window.addEventListener("appinstalled", () => {
    setText("edgeInstallStatus", "Instalada");
    setText("pwaStatusText", "Instalada");
  });

  window.installShitagram = async function(){
    if(isStandalone()){
      alert("O SHITAGRAM™ já está aberto como app.");
      return;
    }
    ...
    if(deferredInstallPrompt){
      deferredInstallPrompt.prompt();
      const choice = await deferredInstallPrompt.userChoice;
      const status = choice.outcome === "accepted" ? "Instalação aceite" : "Instalação recusada";
      setText("edgeInstallStatus", status);
      setText("pwaStatusText", status);
      deferredInstallPrompt = null;
      return;
    }

    alert("No Edge, usa: menu ⋯ → Aplicações → Instalar este site como app.");
  };

  window.copyInstallInstructions = function(){
    const text = `Microsoft Edge no PC:
1. Abre o site.
2. Clica no menu ⋯ no canto superior direito.
3. Vai a Aplicações.
4. Escolhe Instalar este site como app.
5. Confirma Instalar.

Se não aparecer:
- Faz Ctrl+F5.
- Abre edge://apps para ver apps instaladas.
- Em F12 > Application > Service Workers, remove o antigo e recarrega.`;

    navigator.clipboard && navigator.clipboard.writeText(text);
    alert("Instruções copiadas.");
  };

  window.resetPwaEdge = async function(){
    try{
      if("serviceWorker" in navigator){
      ...
      localStorage.setItem("shitagram_pwa_reset", Date.now().toString());
      alert("PWA antigo limpo. Agora a página vai recarregar.");
      location.reload(true);
    }catch(err){
      alert("Não consegui limpar automaticamente. Usa F12 → Application → Service Workers → Unregister.");
    }
  };

  window.openEdgeApps = function(){
    alert("Escreve isto na barra do Edge: edge://apps");
  };

  async function registerSW(){
    setText("edgeHttpsStatus", location.protocol === "https:" ? "HTTPS ativo" : "Precisa de HTTPS");

    const manifest = document.querySelector('link[rel="manifest"]');
    setText("edgeManifestStatus", manifest ? "Manifest ligado" : "Manifest ausente");
    setText("manifestStatusText", manifest ? "Encontrado" : "Não encontrado");

    if(!("serviceWorker" in navigator)){
      setText("edgeSwStatus", "Não suportado");
      setText("swStatusText", "Não suportado");
      return;
    }
    ...
    try{
      const reg = await navigator.serviceWorker.register("./service-worker.js", {scope:"./"});
      await navigator.serviceWorker.ready;
      setText("edgeSwStatus", "Ativo");
      setText("swStatusText", "Ativo");
      console.log("[SHITAGRAM Edge PWA] Service Worker ativo:", reg.scope);
    }catch(err){
      console.error("[SHITAGRAM Edge PWA] Erro:", err);
      setText("edgeSwStatus", "Erro ao ativar");
      setText("swStatusText", "Erro ao ativar");
    }

    if(isStandalone()){
      setText("edgeInstallStatus", "Já instalada");
      setText("pwaStatusText", "Instalada");
    }else{
      setText("edgeInstallStatus", deferredInstallPrompt ? "Pronto para instalar" : "Aguarda evento do Edge");
    }
  }
